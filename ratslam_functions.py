...
class SLAM_basics():
    ...
    def __init__(self):
        ...
        self.sigma = 1
        self.y_size = 640
        self.PC_XY_SUM_SIN_LOOKUP = np.sin(np.multiply(range(1, self.gc_x_dim+1), (2*np.pi)/self.gc_x_dim))
        self.PC_XY_SUM_COS_LOOKUP = np.cos(np.multiply(range(1, self.gc_x_dim+1), (2*np.pi)/self.gc_x_dim))
